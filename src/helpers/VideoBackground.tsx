export const VideoBackground = (settings, type): any => {
  if (settings[type + '_background'].bgType == 'video') {
    if (settings[type + '_background'].videoType == 'local') {
      if (settings[type + '_background'].bgVideo.url) {
        return (
          <div className="pb-video-bg-wrap">
            <video className="pb-video-bg" autoPlay muted loop>
      ...
      if (settings[type + '_background'].bgExternalVideo) {
        ...
        if (video.match('youtube|youtu.be')) {
          ...
          src =
            ...
            '?playlist=' +
            id +
            '&iv_load_policy=3&enablejsapi=1&disablekb=1&autoplay=1&controls=0&showinfo=0&rel=0&loop=1&wmode=transparent&widgetid=1';
